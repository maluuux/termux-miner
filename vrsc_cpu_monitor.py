...
class VrscCpuMinerMonitor:
    ...
    def parse_miner_output(self, line):
        ...
        for pattern in patterns['difficulty']:
            ...
            # ต้องมีส่วนที่ดึงค่า accepted/rejected
    ...
    def display_dashboard(self, miner_data):  # ต้องรับพารามิเตอร์ miner_data
        ...
        # ตรวจสอบและกำหนดค่าเริ่มต้นถ้า miner_data เป็น None
...
def run(self):
    try:
        ...
        for line in iter(process.stdout.readline, ''):
            miner_data = self.parse_miner_output(line)  # เรียก parse_miner_output
            if miner_data:  # ตรวจสอบว่ามีข้อมูล
                self.display_dashboard(miner_data)  # ส่งข้อมูลไปแสดงผล
