...
class VrscCpuMinerMonitor:
                    ...
                    with open(path, 'r') as f:
                        ...
                        if 'pools' in loaded_config and isinstance(loaded_config['pools'], list):
                            if len(loaded_config['pools']) > 0 and isinstance(loaded_config['pools'][0], dict):
                                loaded_config['pools'] = [pool['url'] for pool in loaded_config['pools'] if
                                                          'url' in pool]
    ...
    def parse_miner_output(self, line):
        ...
        for pattern in patterns.get('accepted_rejected', []):
            ...
            try:
                if pattern.pattern == r'accepted\s*:\s*(\d+)\s*/\s*(\d+)':
                    ...
                    results['rejected'] = int(match.group(2)) - accepted
                ...
                print(f"Error processing shares: {e}")
                continue  # หรือจะใช้ break ก็ได้ถ้าต้องการออกจากลูปเมื่อ error
...
def display_dashboard(self, miner_data):
    """แสดงผลข้อมูลการขุด"""
    # กำหนดค่า current_diff จาก miner_data ก่อนใช้งาน
    current_diff = miner_data.get('difficulty')  # ใช้ .get() เพื่อป้องกัน KeyError

    # จากนั้นค่อยตรวจสอบค่า
    if current_diff is not None:
        # โค้ดส่วนแสดงผล difficulty
        ...
        print(
            f"  {COLORS['yellow_bg']}{COLORS['black_text']}Difficulty {COLORS['reset']}: {COLORS['yellow']}ไม่พบข้อมูล{COLORS['reset']}")


# แสดงผล Shares/Accepted/Rejected (ส่วนที่แก้ไข)
def display_dashboard(self, miner_data):  # ต้องรับพารามิเตอร์ miner_data
    """แสดงผลข้อมูลการขุด"""
    # ตรวจสอบว่า miner_data เป็น dictionary และมี key 'accepted' หรือ 'rejected'
    if not isinstance(miner_data, dict):
        miner_data = {}  # กำหนดค่าเริ่มต้นถ้าไม่ใช่ dictionary

    # ตรวจสอบการมีอยู่ของ 'accepted' และ 'rejected' อย่างปลอดภัย
    has_share_data = 'accepted' in miner_data or 'rejected' in miner_data
